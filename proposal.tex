...
\caption{{\bf Left:} mass map, from \citet{Vale2003} simulations, zoomed in on a distended $~10^{15}M_{\odot}$ cluster. {\bf Center:} Aperture mass map with shape noise properties equivalent to existing Subaru observations of A2261. {\bf Right:} Aperture mass map with shape noise properties equivalent to proposed TFR lensing survey. In both cases, we used a compensated aperture mass filter as in \citet{Schneider1998} with $\ell=1$ and the filter scale set to $3$ arcminutes. Peak values in both mass maps are at $\kappa=.08$}
\label{fig:massmaps}
\end{figure}
