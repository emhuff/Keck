...
\caption{\footnotesize {\bf Left:} mass map, from \citet{Vale2003} simulations, zoomed in on a distended $~10^{15}M_{\odot}$ cluster. {\bf Center:} Aperture mass map with shape noise properties equivalent to existing Subaru observations of A2261. {\bf Right:} Aperture mass map with shape noise properties equivalent to proposed TFR lensing survey. Aperture scale is tuned for each map to optimize signal-to-noise and resolution. Peak $\kappa$ values are approximately .07 for both maps.}
\label{fig:massmaps}
\end{figure}
